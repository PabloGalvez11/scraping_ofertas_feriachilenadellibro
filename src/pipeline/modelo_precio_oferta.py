...
def modelo_precio_oferta():
    ...
    df = df.drop(columns=["titulo","categoria", "autor", "descripcion", "link", 
                            "porcentaje_descuento","rango_descuento",
    ...
    X = df.drop("precio_oferta", axis=1)
    y = df["precio_oferta"]
    ...
    joblib.dump(modelo, os.path.join(MODELS_DIR, f"modelos_precio_oferta/modelo_precio_oferta_{fecha_scraping}.joblib"), compress=3)
